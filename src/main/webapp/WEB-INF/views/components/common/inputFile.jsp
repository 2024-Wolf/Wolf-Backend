	...
	<div class="inputGroup">
		...
		<div class="fileGroup">
			<input class="textContent" type="file" name="addFile" id="addFile" style="margin-top: 10px;"
				accept="image/jpeg, image/png" multiple <%=request.getRequestURI().contains("Detail") ? "disabled" :
				(request.getRequestURI().contains("notice") ? "" : "required" ) %>>
			<div id="imageContainer" class="inputGroup imagePlaceholder">
				<!-- 파라미터로 이미지 경로를 전달했는지 확인 -->
				<% String inputFile=request.getParameter("inputFile"); if (inputFile==null || inputFile.isEmpty()) {
					<!-- 전달된 이미지 경로 X, 임시 이미지 선택 -->
					%>
